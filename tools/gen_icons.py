...
and GUI-driven). Outputs:
  ...
  desktop/src/app_icon_macos.h                                    Dock PNG blob
...
def gen_macos(src):
    """macOS Dock tile. The harness is a bare executable, not an .app bundle —
    there is no Info.plist/.icns for the Dock to read — so the Cocoa glue sets
    NSApp.applicationIconImage at runtime (metal_layer_glue.mm), the macOS
    analog of glfwSetWindowIcon on X11/Win32. One 256 px PNG blob (the Dock
    renders tiles at up to 128 pt @2x); NSImage decodes PNG natively, and the
    compressed blob keeps the header ~5x smaller than raw RGBA."""
    path = "desktop/src/app_icon_macos.h"
    buf = io.BytesIO()
    rounded_square(src, 256).save(buf, "PNG", optimize=True)
    data = buf.getvalue()
    lines = [
        "// app_icon_macos.h — GENERATED by tools/gen_icons.py from images/midi-sink.jpg.",
        "// Do not edit by hand; re-run the generator instead.",
        "//",
        "// The 256 px rounded artwork as a PNG blob for the macOS Dock tile:",
        "// the harness is a bare executable (no .app bundle, no .icns), so the",
        "// host glue sets NSApp.applicationIconImage from these bytes at",
        "// startup (sumi_macos_set_dock_icon, metal_layer_glue.mm).",
        "#pragma once",
        "",
        "#include <stdint.h>",
        "",
        f"static const uint8_t sumi_icon_macos_png[{len(data)}] = {{",
    ]
    for i in range(0, len(data), 24):
        chunk = ", ".join(str(b) for b in data[i:i + 24])
        lines.append(f"    {chunk},")
    ...
    with open(path, "w") as f:
        f.write("\n".join(lines))
    print(f"  {path}  (256 px PNG, {len(data)} bytes)")
...
def main():
    ...
    print("ios:")
    gen_ios(src)
    print("linux desktop theme:")
    gen_linux_theme(src)
    print("desktop harness:")
    gen_desktop(src)
    print("macos dock:")
    gen_macos(src)
    print("done.")
