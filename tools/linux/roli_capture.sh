...
#     tools/linux/roli_capture.sh [seconds] [out.csv]
# Captures every ALSA sequencer port matching "ROLI" into out.csv (default
# roli_capture.csv in the current directory) while you play (the app may run at
...
set -euo pipefail
SECS="${1:-20}"; OUT="${2:-roli_capture.csv}"
