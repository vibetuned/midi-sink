...
* **Y is BIPOLAR (fingers, v0.4): up → channel pressure (0xD0) → ink feed; down → polyphonic key pressure (0xA0, the voice's note on its member channel) → Lamb–Oseen swirl.** Touch-down = center = both zeros; one radial soft-knee serves both halves; `value = clamp(|Δy_eff|, 0, 1) · 127` on whichever half is engaged (0–127 — MIDI has no 128). Push away = feed, pull back = stir. Both effects are live simultaneously with no mode flip, both are distinct MIDI messages — so the loopback conformance property holds and DAWs record the swirl dimension. **Honesty note:** 0xD0 is *the* MPE press dimension; many MPE synths ignore 0xA0 entirely — the down axis is primarily a visualizer dimension that happens to be recordable; a synth responding to poly AT is a bonus, never a promise. Rationale for pressure on Y at all stands from the earlier revision: pressure is the engine's hero dimension, glass fingers have no real Z, and the reference hardware speaks bend + pressure (no CC74 — Seaboard territory). Axes stay perceptually orthogonal: vibrato wiggles X, intensity pushes Y.
...
| :-- | :-- | :-- | :-- |
| iOS finger | **Synthesized** — default 96, optionally modulated by `majorRadius` at touch-down (coarse) | **Bipolar Y travel** (§3.3): up = 0xD0 ink feed, down = 0xA0 swirl — full-resolution, deliberate, no hardware estimate involved | No force API: 3D Touch is gone. `majorRadius` is used for optional velocity synthesis only, never for continuous pressure. |
...
| Android finger | Synthesized (as iOS) unless the panel reports real pressure at impact | **Bipolar Y travel** (§3.3), same as iOS — panel `getPressure()` is ignored for continuous control even where real, so both platforms play identically | OEM panel variance stops mattering: the axis is the sensor. |
...
* **Outbound → transports: change-only filtering (never resend an identical PB/CC74/pressure/poly-pressure value per channel), then a PER-TRANSPORT budget** — 0xA0 poly pressure is a continuous dimension like the rest, under the same per-voice decimation and BLE budget — one outbound policy cannot serve links whose capacities differ by an order of magnitude:
...
* **Widget types:**
  * **Spring wheel** — deflection maps to value while held; on release, ramps back to center over ~50 ms (never a jump: a snap is a zipper on the outbound pipe) and guarantees a final center message. Default: **Pitch** (master-channel bend, ±2 — the MPE master default; the strip does not send RPN 0 on ch 1). Master bend keeps its shear tine in every bend_mode — `bend_mode` governs the PER-NOTE bend (DECISIONS_3 #35), so this wheel is unaffected by the ripple toggle. The ripple's external handles are CC 102 (amp, shared with note-bend last-writer-wins) and CC 103 (wavelength) — natural assignments for the two assignable latch wheels.
