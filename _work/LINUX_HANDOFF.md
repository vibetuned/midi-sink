...
1. `CLAUDE.md` — the working rules. **Agents never commit** (prepare
   ...
   number: 67**; the Windows machine may be writing in parallel — coordinate
   numbering through the author), and where a spec line and reality disagree
   you flag it in the entry.
2. `docs/evidence/step33/SUMMARY.md` — batches 1–10, what each changed, how
   it was verified on the Mac, and which Android files are uncompiled.
3. `_work/DECISIONS_4.md` #49–#66 — the record of what shipped. Android is
   named in #49, #53, #54, #56, #60, #64, #65; the desktop in #57–#59.
...
* **Core ABI 0.5 → 0.8.0** — rebuild everything from scratch, desktop and
  ...
  and vortex profile enums (#49). Desktop: `cmake -B build -G Ninja && cmake
  --build build && ctest --test-dir build` — four suites, 18 377 mapper
...
* **Behaviour every platform shares:** MPE mode gives a channel-1 keyboard
  ...
  **CC 64 never dips the paper in any mode** (#62); wind mode is MPE plus a
  ...
  the bend-driven ripple is 4× more sensitive (#66); the measured Airwave CC
  map (#50).
...
2. **The borderless canvas (#59).** The canvas is created with
   `GLFW_DECORATED` off. On **Wayland** that is a request to the compositor
   (libdecor / xdg-decoration): report what GNOME actually does — no frame,
   Super + drag still moves it, Super + Up/Down, Alt + F4 closes, Ctrl , opens
   the framed settings window. Repeat on **X11** (`WAYLAND_DISPLAY= …`). If the
   borderless canvas is unmanageable on either, say so with the concrete
   failure — the recorded fallback is a **per-platform default, not a
   setting** (#59).
3. **Fullscreen (#58).** Settings › Window › Fullscreen and F11 on both
...
4. **Input mode (#60/#62/#63)** with the ROLI over ALSA and any plain
...
5. **Layouts (#64).** Eight in the combo; the four Piano rolls (left / right /
...
7. **Packaging unaffected?** `cmake/LinuxPackaging.cmake` did not change; one
   local CPack DEB, install into a clean container, `--version`, launch — to
   be sure nothing in the new flags or the borderless window upsets the
   `.desktop` launch.
