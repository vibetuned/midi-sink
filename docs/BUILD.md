...
Everything a developer needs to build, run, test and package the engine and
its shells. Users install from the channels listed in the
[README](../README.md) and read the [documentation site](https://midi-sink.vibetuned.com/);
the full specification is [PROJECT_SPEC.md](PROJECT_SPEC.md), the decision log
[DECISIONS.md](DECISIONS.md), the history [CHANGELOG.md](CHANGELOG.md) and
[ROADMAP.md](ROADMAP.md). The scripts under `tools/` are described in
...
On macOS the icon is the bundle's `.icns` (`packaging/macos/midi-sink.icns`,
written by the same script) — the `.app` bundle replaced the v0.4 runtime
Dock-tile hack in Step 23 (DECISIONS_4 #6).

## CC routing through the C ABI
...
A channel-specific route overrides an any-channel route. CC 74 on MPE member
channels (slide) is consumed before the map; CC 64 is an ordinary, unmapped
controller (DECISIONS_4 #62).
